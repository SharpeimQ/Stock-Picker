...
# Make a replica of the original array and remove the min and max values from the replica until the index of the min is
# less than the index of the max
...
def stock_picker(stock, replica)
  ...
  [stock.find_index(min), stock.find_index(max)]
end
